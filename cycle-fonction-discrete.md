...
Le sujet de cet article est l'étude des cycles d'une fonction définie sur un ensemble $X \subseteq \mathbb{N}$. Pour ce faire nous allons introduire une correspondance entre la fonction $f:X \to X$ et une matrice qui lui est associée, $\operatorname{Adj} f$, nommée la matrice d'adjacence de $f$. Cette matrice a la particularité d'agir sur les vecteurs unitaires naturels $\vec e_i$ d'une manière isomorphe à la fonction $f$ sur son ensemble de définition. 
